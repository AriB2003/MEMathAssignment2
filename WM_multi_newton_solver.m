...
function [x, exit_flag] = WM_multi_newton_solver(fun,x_guess,solver_params)
    ...
    % Calculate initial Xn Jxn Fxn values
    Xn = x_guess;
    if numerical_diff
        Fxn = fun(Xn);
        Jxn = WM_approx_jacobian(fun, Xn);
    else
        [Fxn, Jxn] = fun(Xn);
    end
    ...
    while any(abs(Fxn)>ftol) && det(Jxn*Jxn')~=0 && iterations<1000
        ...
        % Update Xn based on rule
        Xn = Xn-Jxn\Fxn;

        % update Fxn and Jxn accordingly
        if numerical_diff
            Fxn = fun(Xn);
            Jxn = WM_approx_jacobian(fun, Xn);
        else
