params = struct();
[roots, flag] = multi_newton_solver(@compute_distance, [pi/4;3], params)
run_simulation(roots(1),roots(2))

function dist = compute_distance(X)
    dist = projectile_traj(X(1),X(2))-target_traj(X(2));
end
