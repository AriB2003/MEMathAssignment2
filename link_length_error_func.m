...
function length_errors = link_length_error_func(vertex_coords, leg_params)
    ...
    for i=1:length(length_errors)
        a = leg_params.link_to_vertex_list(i,1); % node number of link start
        b = leg_params.link_to_vertex_list(i,2); % node number of link end
        vertex_matrix = column_to_matrix(vertex_coords);
        length_errors(i) = (vertex_matrix(b,1)-vertex_matrix(a,1))^2 + (vertex_matrix(b,2)-vertex_matrix(a,2))^2 - leg_params.link_lengths(i)^2;
    end
end